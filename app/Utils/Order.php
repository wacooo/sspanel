...
function buildRequestFormShan($para_temp, $key){
	callPayGateway($para_temp, "http://www.passpay.net/PayOrder/payorder", $key);
}
 ...
 *
 */
function callPayGateway($para_temp, $gateway, $key) {
	//待请求参数数组
	$para = buildRequestParaShan($para_temp,$key);

	$sHtml = "<form id='paysubmit' name='paysubmit' action='".$gateway."' accept-charset='utf-8' method='POST'>";
